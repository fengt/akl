...
public class DGYRKDPrinterWeb extends ActionsoftWeb {
	/**
	 * 查询预入库单的打印次数.
	 */
	private static final String QUERY_RKD_YRKDDYCS = "SELECT YDYCS FROM BO_AKL_DGRK_P WHERE BINDID=?";
	/**
	 * 更新预入库单的打印次数.
	 */
	private static final String UPDATE_RKD_YRKDDYCS = "UPDATE BO_AKL_DGRK_P SET YDYCS=? WHERE BINDID=?";
	/**
	 * 打印纸每页行数.
	 */
	public static final int PAGE_SIZE = 10;
	
	public DGYRKDPrinterWeb() {
	}

	public DGYRKDPrinterWeb(UserContext arg0) {
		super(arg0);
	}
	...
	public String parseHtml(String bindid){
		...
		String bodysql = "SELECT WLBH,WLMC,XH,DW,SX,YSSL,SSSL,HWDM FROM BO_AKL_DGRK_S WHERE BINDID = '"+bindid+"' order by XH";
		...
		try {
			 ...
			 * 获取打印次数并更新打印次数.
			 */
			Integer yrkddycs = DAOUtil.getIntOrNull(conn, QUERY_RKD_YRKDDYCS, bindid);
			if (yrkddycs == null) {
				yrkddycs = 0;
			}
			DAOUtil.executeUpdate(conn, UPDATE_RKD_YRKDDYCS, ++yrkddycs, bindid);
			try{
				...
				// 填充主表数据
				fillMainInfo(conn, bindid, yrkddycs, hashtable);
				...
				for (int i = 0; i < trList.size(); i++) {
					/*if (i != 0 && i % PAGE_SIZE == 0) {
						...
						htmlBuilder.append(getHtmlPage("代管预入库单.html", hashtable));
	 ...
	 * @throws SQLException
	 */
	private void fillMainInfo(Connection conn, String bindid, Integer yrkddycs, Hashtable<String, String> hashtable)
			throws SQLException {

		PreparedStatement headPs = null;
		ResultSet headRs = null;
		String headsql = "SELECT RKDH,KHBH,SHCK,BZ FROM BO_AKL_DGRK_P WHERE BINDID = '"+bindid+"'";
...
//				String shck = headRs.getString("SHCK");//收货仓库
				...
				hashtable.put("YDYCS", String.valueOf(yrkddycs));
...
//				hashtable.put("ZL", "");
	...
	private StringBuilder parseTableTRHtml(ResultSet bodyRs, int bh) throws SQLException {
		...
		sb.append(formatBodyRowRecord(parseNull(bhxlh),parseNull(xh),parseNull(wlmc),parseNull(sxmc),parseNull(dwmc),parseNull(yssl),parseNull(sssl),parseNull(hwdm)));
		sb.append("</tr>");
		return sb;
	}
