...
public class Validate_FK extends ExcelDownFilterRTClassA {


	public Validate_FK(UserContext uc) {
		super(uc);
		...
		try {
			...
			if(rs != null){
				while(rs.next()){
					...
					while(it.hasNext()){
						...
						String fkxh = formData.get("XH") == null ?"":formData.get("XH").toString();//反馈型号
						String fksl = formData.get("SL") == null ?"":formData.get("SL").toString();//反馈数量
						int fsl = Integer.parseInt(fksl);//反馈数量
			...
			if(!fkxhlist.isEmpty()){
				...
				while(t.hasNext()){
					...
					if(!flag){
						//匹配反馈表物料号
						String wlsql = "select WLBH from BO_AKL_WLXX where XH='"+s1+"' and HZBM='01065'";
						String wlh = DBSql.getString(wlsql, "WLBH");
						//更新差异数
