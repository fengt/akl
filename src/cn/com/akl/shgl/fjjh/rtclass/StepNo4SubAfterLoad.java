...
public class StepNo4SubAfterLoad extends SubWorkflowEventClassA {
	...
	public StepNo4SubAfterLoad() {
	}

	public StepNo4SubAfterLoad(UserContext arg0) {
		super(arg0);
		this.uc = arg0;
		setProvider("fengtao");
	...
	public boolean execute() {
		...
		try {
			...
			/**加载数据到调拨单.*/
			String fhck = DAOUtil.getStringOrNull(conn, FJJHCnt.QUERY_FJJH_P_FHCKBM, parent_bindid);//返货库
			Vector<Hashtable<String, String>> vector = fjjhBiz.queryByWlbh(conn, parent_bindid, fhck);
			BOInstanceAPI.getInstance().createBOData(conn, "BO_AKL_DB_HZ_S", vector, sub_bindid, uid);//加载子表数据
			dbBiz.insertDBHead(conn, parent_bindid, sub_bindid, uid, FJJHCnt.direction0);//加载主表数据
