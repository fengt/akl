 ...
 *
 */
public class FXCKDPrinterWeb extends ActionsoftWeb {
	...
	public FXCKDPrinterWeb() {
		super();
	}

	public FXCKDPrinterWeb(UserContext arg0) {
		super(arg0);
	}
			...
			for (String fkck : fkcklist) {
				...
				try {
					...
					for (int row = 1; bodyReset.next(); row++) {
						// 加载子表单身信息
						trList.add(parseTableTRHtml(bodyReset, row, hashtable.get("RMAFXDH"), hashtable.get("KH")));
					...
					for (int i = 0; i < trList.size(); i++) {
						if (i != 0 && i % PAGE_SIZE == 0) {
							...
							htmlBuilder.append(getHtmlPage("返新出库单.html", hashtable));
