 ...
 *
 */
public class FXBHDPrinterWeb extends ActionsoftWeb {
	 ...
	 * 查询出库单单身.
	 */
	private static final String QUERY_CKD_BODY = "SELECT WLH, XH, SX, JLDW, WLMC, KWBH, PC, FHKFMC, SUM(ISNULL(SL, 0)) SL, SUM(ISNULL(SJSL, 0)) SJSL FROM BO_AKL_CKD_BODY WHERE BINDID=? AND FHKFBH=? GROUP BY WLH, XH, SX, JLDW, WLMC, KWBH, PC, FHKFMC ORDER BY KWBH, XH";
	/**
	 * 查询出库单发货仓库.
	 */
	 ...
	 * 查询出库单中预出库单的打印次数.
	 */
	private static final String QUERY_CKD_YCKDDYCS = "SELECT YCKDDYCS FROM BO_AKL_CKD_HEAD WHERE BINDID=?";
	/**
	 * 更新出库单中的预出库单打印次数.
	 */
	private static final String UPDATE_CKD_YCKDDYCS = "UPDATE BO_AKL_CKD_HEAD SET YCKDDYCS=? WHERE BINDID=?";
	
	private static final int PAGE_SIZE = 30;

	public FXBHDPrinterWeb() {
		super();
	}

	public FXBHDPrinterWeb(UserContext arg0) {
		super(arg0);
	}
			...
			for (String fkck : fkcklist) {
				...
				try {
					...
					for (int row = 1; bodyReset.next(); row++) {
						// 加载子表单身信息
						trList.add(parseTableTRHtml(bodyReset, row, hashtable.get("RMAFXDH")));
					...
					for (int i = 0; i < trList.size(); i++) {
						if (i != 0 && i % PAGE_SIZE == 0) {
							...
							htmlBuilder.append(getHtmlPage("返新出库备货单.html", hashtable));
	...
	private void fillMainInfo(Connection conn, String bindid, String today, Integer yckddycs, Hashtable<String, String> hashtable)
		...
		try {
			...
			if (headReset.next()) {
				...
				hashtable.put("RMAFXDH", PrintUtil.parseNull(headReset.getString("RMAFXDH"))); // RMA返新单号
				hashtable.put("BZ", PrintUtil.parseNull(headReset.getString("BZ"))); // 备注
				...
				hashtable.put("YCKDDYCS", yckddycs.toString()); // 发货公司
			}
		} finally {
	 ...
	 * @throws SQLException
	 */
	private StringBuilder parseTableTRHtml(ResultSet bodyReset, int row, String khcgdh) throws SQLException {
...
//		sb.append(PrintUtil.formatBodyRowRecord(bodyReset.getString("WLMC")));// 物料名称
		sb.append(PrintUtil.formatBodyRowRecord(bodyReset.getInt("SL")));// 实际数量
		sb.append(PrintUtil.formatBodyRowRecord(bodyReset.getString("KWBH")));// 货位代码
		sb.append(PrintUtil.formatBodyRowRecord(bodyReset.getString("PC")));// 批次
		//sb.append(PrintUtil.formatBodyRowRecord(bodyReset.getInt("SL")));// 订单数量
	...
	private Integer updatePrintCount(String bindid, Connection conn) throws SQLException {
		 ...
		 * 获取打印次数并更新打印次数.
		 */
		Integer yckddycs = DAOUtil.getIntOrNull(conn, QUERY_CKD_YCKDDYCS, bindid);
		if (yckddycs == null) {
			yckddycs = 0;
		}
		DAOUtil.executeUpdate(conn, UPDATE_CKD_YCKDDYCS, ++yckddycs, bindid);
		return yckddycs;
	}
}
