 ...
 *
 */
public class BHDPrinterWeb extends ActionsoftWeb {
	...
	public BHDPrinterWeb() {
		super();
	}

	public BHDPrinterWeb(UserContext arg0) {
		super(arg0);
	}
			...
			for (String fkck : fkcklist) {
				...
				try {
					...
					for (int row = 1; bodyReset.next(); row++) {
						// 加载子表单身信息
						trList.add(parseTableTRHtml(bodyReset, row, hashtable.get("KHCGDH")));
					...
					for (int i = 0; i < trList.size(); i++) {
						if (i != 0 && i % PAGE_SIZE == 0) {
							...
							htmlBuilder.append(getHtmlPage("网销出库备货单.html", hashtable));
