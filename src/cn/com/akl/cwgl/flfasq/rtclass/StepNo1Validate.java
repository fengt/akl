...
public class StepNo1Validate extends WorkFlowStepRTClassA {
	 ...
	 * 查询重复的物料信息.
	 */
	private static final String QUERY_WLBH_REPEAR_COUNT = "SELECT WLBH FROM BO_AKL_WXB_XS_FL_BODY WHERE BINDID=? GROUP BY WLBH HAVING COUNT(*)>1";

	/**
	 * 查询物料编号和型号不匹配的型号.
	 */
	private static final String QUERY_WLXX_NOMATCH = "SELECT a.XH FROM BO_AKL_WXB_XS_FL_BODY a LEFT JOIN BO_AKL_WLXX b ON a.WLBH=b.WLBH AND a.XH=b.XH WHERE a.BINDID=? AND (b.WLBH is null OR b.XH is null)";
